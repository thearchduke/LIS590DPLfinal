                ...
                <div class="left">
                    ...
                    <xsl:for-each select="a[contains(concat($cocoonBase, substring-before(., '.xml'), '/text'), 'ih')]">
                        ...
                        <xsl:if test="$votes/vote[@bill=$billNum and contains(@datetime, '2009')]">
                            <p>
                                <a href="{$thisUrl}">Transformation of: 
                                    <xsl:variable name="dc" select="document(concat('http://thomas.loc.gov/home/gpoxmlc111/', .))/bill/metadata/dublinCore/dc:title"></xsl:variable>
                                    <xsl:if test="not($dc='')">
                                        <xsl:value-of select="document(concat('http://thomas.loc.gov/home/gpoxmlc111/', .))/bill/metadata/dublinCore/dc:title"></xsl:value-of>
                                    </xsl:if>
                                    <xsl:if test="$dc=''">
                                        <xsl:value-of select="document(concat('http://thomas.loc.gov/home/gpoxmlc111/', .))/bill//short-title[1]"></xsl:value-of>
                                    </xsl:if></a>
